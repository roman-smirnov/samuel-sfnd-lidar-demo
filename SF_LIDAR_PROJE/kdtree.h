...
struct KdTree
{
	...
	void insert(std::vector<float> point, int id)
	{
		...
		instertHelper(&root,0,point,id);

	}

	void instertHelper(Node** node, uint depth, std::vector<float> point, int id){
		...
		*node = new Node(point,id);
		else
		{
			...
			if (point[cd] < ((*node)->point[cd]))
				instertHelper(&((*node)->left),depth+1,point,id);
			else
				instertHelper(&((*node)->right),depth+1,point,id);
		}
