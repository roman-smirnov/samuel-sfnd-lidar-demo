...
namespace process_point_clouds {
...
typename pcl::PointCloud<PointT>::Ptr FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud,
  ...
  //pcl::PointIndices inliers2 {new pcl::PointIndices};
...
std::pair<typename pcl::PointCloud<PointT>::Ptr,
  ...
  std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>
      segResult(obstCloud, planetCloud);
  return segResult;
}

template<typename PointT>
std::unordered_set<int> Ransac3dHelper(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
                                       int maxIterations,
                                       float distanceTol) {
...
template<typename PointT>
std::pair<typename pcl::PointCloud<PointT>::Ptr,
          typename pcl::PointCloud<PointT>::Ptr> MySegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud,
                                                                int maxIterations,
                                                                float distanceThreshold) {
  // Time segmentation process
  auto startTime = std::chrono::steady_clock::now();

  std::unordered_set<int> inliers = Ransac3dHelper(cloud, 2, 1.0);
  typename pcl::PointCloud<PointT>::Ptr cloudInliers(new pcl::PointCloud<PointT>);
  typename pcl::PointCloud<PointT>::Ptr cloudOutliers(new pcl::PointCloud<PointT>);

  for (int index = 0; index < cloud->points.size(); index++) {
    //check to see if index is in the inliers, otherwise add it to outliers
...
std::pair<typename pcl::PointCloud<PointT>::Ptr,
  ...
  std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr>
      segResult = SeparateClouds<PointT>(inliers, cloud);
  return segResult;
}
